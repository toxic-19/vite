...
const vDrag: Directive = (el: HTMLElement) => {
  ...
  const _index = (element: HTMLElement) => {
    ...
    return domData.findIndex(
      (dom: HTMLElement) => dom.innerText == element.innerText
    )
  }
...
const imageList: Record<string, { default: string }> = import.meta.globEager(
  "../../assets/lazy-images/*.*"
) // 静态资源加载 类似于 import XX from ""
